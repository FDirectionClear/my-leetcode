/**
 * 什么场景下使用回溯？
 * （1）穷举：回溯法可以看成暴力法的升级版，它从解决问题的所有可能选项里系统地选择出一个可行的解决方案。所以需要穷举出所有情况，并从所有情况中选择策略的题型可以有限考虑使用回溯。
 * 回溯法非常适合由多个步骤组成的问题，并且每个步骤都有多个选项。当我们在某一步选择了其中一个选项时，就进入下一步，然后又面临新的选项。一直这样重复选择，直至到达最终状态。
 *
 * （2）树形结构：用回溯法解决问题的所有选项可以形象地用树状结构表示。在某一步有n个可能的选项，那么该步骤可以看成是树状结构中的一个节点，
 * 每个选项看成树中节点连接线，经过这些连接线到达该节点的n个子节点。树的叶节点对应着终结状态。如果在叶节点的状态满足题目
 * 的约束条件，那么我们找到一个解决方法。
 *
 * （3）有时候会关心决策的结果具体是什么，这点和动态规划和贪心不太一样。
 *    eg：
 *       动态规划：最后能不能？最大是多少？一共有几种？ 他不需要让你拿出对应结果的例子，也就是决策的不关注过程。
 *       回溯：请返回所有答案的结果？它不仅要你判断能不能？有多少种？他还需要你拿出结果。
 *    Tip：并不是所有回溯的场景都是这样明显，有可能看上去也和动态规划一样不关注过程。但是目前见到的所有动态规划的题目都不关心过程。
 */
...
var generateParenthesis = function (n) {
  ...
  function backtrack(left, right, track, res) {
    // 数量小于0，不合法
    if (left < 0 || right < 0)
      return; /** 回溯终止条件1，退出条件，说明当前子问题没有后续潜力，不符合要求，会回到上一个循环 */
    // 若左括号剩下的多，说明不合法
    if (right < left)
      return; /** 回溯退出条件2,退出条件，说明当前子问题没有后续潜力，不符合要求，会回到上一个循环 */
    // 所有括号用完，得到合法组合
    if (left == 0 && right == 0) {
      /** 求解条件，当left === right === 0的时候，就是解了，把它推进结果数组里。*/
      res.push(track.join(""));
      return;
    }
