...
var combine2 = function (n, k) {
  const arr = Array.from(new Array(n), (_, index) => index + 1);
  const helper = (group, needs) => {
    if (needs === 1) return [group];
    const res = [];
    for (let i = 0, len = group.length; i < len; i++) {
      const subCombin = helper(group.slice(i + 1), needs - 1);
      for (let j = 0, l = subCombin.length; j < l; l++) {
        res.push(subCombin[j].unshift(arr[i]));
      }
    }
    return res;
  };
